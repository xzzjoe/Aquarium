...
class GLProgram:
    ...
    def __init__(self) -> None:
        ...
        for i in range(int(self.attribs["maxLightsNum"])):
            ...
            self.attribs[f"light[{i}].contribute"] = f"{self.attribs['light']}[{i}].contribute"
    ...
    def genFragShaderSource(self):
        ...
        struct Material{{
            ...
            vec4 diffuse;
            vec4 specular;
            float highlight; //Assumingt this is the specular component
        }};
        ...
        out vec4 FragColor;
        uniform bool {self.attribs["ambient_toggle"]};
        uniform bool {self.attribs["diffuse_toggle"]};
        uniform bool {self.attribs["specular_toggle"]};
        void main()
        {{
            ...
            if ((renderingFlag >> 0 & 0x1) == 1){{
                ...
                vec4 illumination = vec4 (0,0,0,0);
                int n = light.length();
                bool ambient_used = false;
                if (!ambient_toggle) ambient_used = true;
                for (int i = 0; i < n; i++){{
                    //ambient
                    Light currLight = light[i];
                    if(!currLight.contribute) continue;
                    vec4 lightColor = currLight.color;
                    vec4 amb = vec4(0,0,0,0);
                    //Diffuse
                    vec3 L;
                    if(currLight.infiniteOn){{
                        L = currLight.infiniteDirection;
                        if (!ambient_used){{
                            amb = material.ambient * lightColor;
                            ambient_used = true;
                        }}

                    }}
                    else L = -1 *(vPos - currLight.position);
                    
                    float f_a = 1;
                    float f_r = 1;

                    vec3 norm_L = normalize(L);
                    vec3 norm_vNormal = normalize(vNormal);
                    if(currLight.spotOn){{
                        vec3 norm_spotD = normalize(currLight.spotDirection);
                        float angularDiff = dot(norm_L, norm_spotD);
                        if (angularDiff > cos(currLight.spotAngleLimit)) f_a = pow(angularDiff, material.highlight);
                        else continue;

                        float dist = distance(currLight.position, vPos);
                        vec3 radialFactor = currLight.spotRadialFactor;
                        f_r = 1.0/(radialFactor[0]*pow(dist,2) + dist*radialFactor[1] + radialFactor[2]);

                    }}
                    float angle_cos = dot(norm_vNormal, norm_L);
                    vec4 diffuse = vec4(0,0,0,0);
                    vec4 specular = vec4(0,0,0,0);
                    if (angle_cos > 0){{
                        // front-faced, has diffuse and specular component.
                        if(diffuse_toggle) diffuse = material.diffuse * currLight.color * angle_cos;
                        //Specular
                        vec3 V = -1 *(viewPosition - vPos);
                        vec3 R = 2 *(angle_cos)*norm_vNormal - norm_L;
                        vec3 normV = normalize(V);
                        vec3 normR = normalize(R);
                        float specular_cos = dot(normV, normR);
                        if (specular_cos > 0 && specular_toggle){{
                            specular = material.specular * currLight.color * pow(specular_cos, material.highlight); 
                        }}
                    }}
                    illumination += amb + (f_a*f_r)*(diffuse + specular);
                }}
                for (int i = 0; i < 3; i++){{
                    result[i] += illumination[i];
                    result[i] = min(1.0, result[i]);
                }}
    ...
    def setLight(self, lightIndex: int, light: Light):
        ...
        self.setBool(f"""{self.attribs["light"]}[{lightIndex}].contribute""", light.contribute, False)
        if(light.infiniteOn):
