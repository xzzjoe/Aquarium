...
class Sketch(CanvasBase):
    ...
    def InitGL(self):
        ...
        self.shaderProg.setBool("ambient_toggle",self.ambient)
        self.shaderProg.setBool("diffuse_toggle",self.diffuse)
        self.shaderProg.setBool("specular_toggle",self.specular)
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if chr(keycode) in "Ss":
            ...
            self.shaderProg.setBool("specular_toggle", self.specular)
            print("specular is currently: ", self.specular)
        if chr(keycode) in "Dd":
            self.diffuse = not self.diffuse
            self.shaderProg.setBool("diffuse_toggle", self.diffuse)
            print("diffuse is currently: ", self.diffuse)
        if chr(keycode) in "Aa":
            self.ambient= not self.ambient
            self.shaderProg.setBool("ambient_toggle", self.ambient)
            print("ambient is currently: ", self.ambient)
        if chr(keycode) in "1":
            self.scene.lights[0].toggleContribute()
            self.shaderProg.setLight(0,self.scene.lights[0])
        if chr(keycode) in "2":
            self.scene.lights[1].toggleContribute()
            self.shaderProg.setLight(0,self.scene.lights[1])
        if chr(keycode) in "3":
            if(2 < len(self.scene.lights)):
                self.scene.lights[2].toggleContribute()
                self.shaderProg.setLight(0,self.scene.lights[2])
        if chr(keycode) in "4":
            if(3 < len(self.scene.lights)):
                self.scene.lights[3].toggleContribute()
                self.shaderProg.setLight(0,self.scene.lights[3])
